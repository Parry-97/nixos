...
#
# The opencode-go kit (home/sbx/opencode-go/spec.yaml) is installed to
# ~/.config/sbx/kits/opencode-go/ via home.file below. It declaratively
# wires feature flags, proxy-managed OPENCODE_API_KEY credential, the
# network allowlist, and auto-configures the context7 MCP server — so an
# `sbx-run opencode` just works without manual env injection.
#
...
{ ... }:
{
  # opencode-go kit — a v1 mixin applied via `sbx run --kit` when the agent
  # is opencode. Installs feature flags (Exa search, LSP tool), proxy-managed
  ...
  programs.bash.initExtra = ''
    ...
    # context7 MCP auto-config). Pass agent args after `--`.
    sbx-run() {
      if [[ "$#" -lt 1 ]]; then
        echo "Usage: sbx-run <agent> [-- AGENT_ARGS...]" >&2
        echo "Agents: claude codex gemini opencode copilot kiro ..." >&2
        return 2
      fi
    ...
    #      The opencode-go kit's network.allowedDomains opens the 3 endpoints
    #      an opencode agent needs, per-sandbox. Fails harmlessly if the
    #      policy is already initialized — check `sbx policy ls`.
    #   2. Stores the OpenCode Go subscription API key in the OS keychain
    #      (or encrypted file fallback on headless hosts). The kit's
    #      credentials.sources reads this — proxy-managed, so the real key
    #      never enters the sandbox. You'll be prompted to paste the key.
    sbx-init() {
